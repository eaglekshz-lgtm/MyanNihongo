...
class LearningModeConfigPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<LearningModeConfigPage> createState() =>
      _LearningModeConfigPageState();
}

class _LearningModeConfigPageState
    extends ConsumerState<LearningModeConfigPage> {
  String? _level;
  String? _wordType;
  // Configuration state
  ...
  Future<void> _loadVocabularyCount() async {
    ...
    final count = await ref.read(
      vocabularyCountByLevelProvider(_level ?? 'N5').future,
    );

    AppLogger.info(
      'Retrieved count: $count vocabulary items',
      'LearningModeConfig',
    );
